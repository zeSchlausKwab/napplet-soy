...
These changes are local until you deploy them. CLI 0.4.0 adds the local Listing preview and screenshot capture controls. The website offers install-and-remix, comment likes/zaps, and an initially empty admin Featured collection. Upload the locally built CLI archives with `bun run cli:release --host root@159.198.46.2` before deploying the website. Existing CLI users then rerun the installer and restart `napplet-space dev`; `napplet-space skills update` refreshes their creator guidance. Readable links require a one-time **Named link** claim on the website and then follow future releases. See [CLI instructions](docs/CLI.md) and [community behavior](docs/COMMUNITY.md).
