...
Implementation update (2026-09-12): the [managed Khatru/LMDB/Bleve relay](RELAY.md) now runs under PM2 in both dev modes, seeded through Applesauce. The VPS script builds the same service; no Docker dependency is required for it. The full topology below remains a design target. See [PUBLIC-DEVELOPMENT.md](PUBLIC-DEVELOPMENT.md) for automatic seeds, relay-only publicdev, and SSR/PNG previews, and [CONTEXTVM.md](CONTEXTVM.md) for the multiplayer service boundary. The remaining infrastructure and extensions below are still a plan. Public clients accept ordinary named/root/snapshot manifests without requiring Space extensions.
