...
State is outside release directories. Local HTTP and public HTTPS use the same service protocols and backends. The index/preview worker shares `infra/indexer.ecosystem.config.cjs` between environments. Trusted local HTTPS remains planned.
