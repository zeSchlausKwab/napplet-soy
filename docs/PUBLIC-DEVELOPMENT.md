...
Only single-file `/index.html` artifacts without required host domains are downloaded for playback. Declared unsupported domains are displayed clearly. Undeclared dependencies in older napplets may still prevent them from working. The client supports the basic opaque-iframe projection and does not claim every draft NAP. This applies equally to our local and production player.

Blossom downloads use signed server hints, verify hashes and UTF-8, reject private/special network destinations at connection-time DNS lookup, and do not follow redirects. Downloads are size-limited to 10 MiB, with three workers, per-mirror timeouts, and a ten-second download window. No remote URL is fetched on a page/OG request and no napplet code executes on the server.

The public cache lives exclusively under `.local/publicdev`, outside the production archive and build assets. It is reused for 15 minutes, refreshed on startup after that, and retained with its original timestamp when relays fail. `--refresh` bypasses the TTL. Public mode requires an explicit runtime switch, so saved public entries and their artifacts return 404 in ordinary local/production mode. Browser requests do not refresh this cache. An ordinary VPS deployment disables publicdev.

An actual read-only smoke run found 93 valid manifests and cached 13 verified artifacts. Rubik Cube played from its relay manifest and Blossom bytes in Chromium with no page errors. Those counts are observations, not guaranteed inventory or proof that every artifact implements its declared capabilities correctly.
