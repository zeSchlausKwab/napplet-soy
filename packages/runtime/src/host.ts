...
export function attachNappletHost(options: HostOptions) {
  ...
  const nostr = new PlaybackNostr(options.relays, send, () => options.pubkey);
  const files = new NappletFiles(send, (value) => {
    if (alive) options.files(value);
  });
  const store = scopedStorage(
    localStorage,
    `${options.identity}:${options.pubkey ?? 'guest'}`,
    crypto.randomUUID(),
  );
  ...
  const handle = async (message: Record<string, unknown>) => {
    ...
    if (!(RUNTIME_DOMAINS as readonly string[]).includes(domain))
      throw new Error(`Unsupported capability: ${domain}`);
  ...
  const listener = (event: MessageEvent) => {
    ...
    if (!initialized || !message.id) return;
    if (message.type === 'resource.cancel') {
      resources.get(message.id)?.abort();
      return;
    }
    let size: number;
    try {
    ...
    if (++calls > 600 || pending >= 32) {
      failure(new Error('Request quota exceeded'));
      return;
    }
    pending++;
    void handle(message)
      .then((result) => send({ type: `${message.type}.result`, id: message.id, ...result }))
      .catch(failure)
      .finally(() => pending--);
  };
